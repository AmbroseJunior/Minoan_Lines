...
type VesselData = {
  ...
  eu_ets_allowances_needed: number;
  cii_score: number;
  fueleu_rating: string;
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-bold text-[#001A4D]">EU Compliance</h1>
        </div>
      ...
      {s && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { label: 'Total CO₂ (tons)', value: s.total_co2_tons.toFixed(0), icon: AlertTriangle, color: 'text-orange-600' },
            { label: 'ETS Allowances', value: s.total_ets_allowances.toFixed(0), icon: FileText, color: 'text-blue-600' },
            { label: 'Avg CII Score', value: s.avg_cii_score.toFixed(1), icon: CheckCircle, color: 'text-green-600' },
            { label: 'Report Date', value: s.report_date, icon: FileText, color: 'text-gray-600' },
      ...
      <div className="card overflow-hidden">
        <div className="p-4 border-b flex items-center justify-between">
          <h2 className="font-semibold text-gray-900">Fleet Compliance Details</h2>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50">
              <tr>
                {['Vessel', 'Fuel (t)', 'CO₂ (t)', 'ETS Needed', 'CII Score', 'FuelEU', 'GHG Intensity'].map(h => (
              ...
              {data?.vessels.map(v => (
                <tr key={v.vessel} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <span className={clsx('badge', ratingColor(v.fueleu_rating))}>{v.fueleu_rating}</span>
                  </td>
      ...
      <div className="card p-4 space-y-3">
        ...
        <div className="flex gap-3 items-center">
          <select value={reportType} onChange={e => setReportType(e.target.value as typeof reportType)}
            ...
            <option value="eu_ets">EU ETS Report</option>
            <option value="fuel_eu">FuelEU Maritime</option>
          </select>
